...
def get_loader(mode='train', load_mode=0,
               ...
               transform=None, batch_size=32, num_workers=6, shuffle=True):
    myData = MyData(mode, load_mode, saved_path, test_patient, patch_n, patch_size, transform)
    data_loader = DataLoader(dataset=myData, batch_size=batch_size, shuffle=shuffle, num_workers=num_workers)
    return data_loader
...
if __name__ == '__main__':
    ...
    for i, traindata in enumerate(myLoader):
        print('i:', i)
        x, y = traindata

        print(x)
        x = np.transpose(x.cpu().numpy(), (1, 2, 0))  # 把channel那一维放到最后
        # 显示图片
        plt.imshow(x)
        plt.show()

        print(y)
        y = np.transpose(y.cpu().numpy(), (1, 2, 0))  # 把channel那一维放到最后
        # 显示图片
        plt.imshow(y)
        plt.show()
